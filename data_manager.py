...
def prepare_returns_matrix(df, tickers):
    ...
    df must have columns: Date, Ticker, Close (or Adj Close).
    Assumes 'Close' column exists – adjust if different.
    """
...
def align_macro_returns(returns, macro):
    ...
    # Drop rows where macro is still NaN (before first macro date)
